...
class DockerPlaywrightBrowser(
    AsyncContextManager["DockerPlaywrightBrowser"], ABC, ComponentBase[BaseModel]
):
    """
    ...
    async def _start(self) -> None:
        ...
        while True:
            try:
                ...
                self._close_container() # Make sure to clean up the failed container
                retries += 1
                if retries >= 3:
                    logger.error("Failed to start container after 3 attempts.")
                    raise
